...
fn create_xmllint(toml: &toml::Value) -> anyhow::Result<Box<dyn crate::processors::Processor>> {
    crate::registries::deserialize_and_create(toml, |cfg| Box::new(SimpleChecker::new(cfg, SimpleCheckerParams { description: "Validate XML files using xmllint", subcommand: Some("--noout"), prepend_args: &[], extra_tools: &[] })))
}
