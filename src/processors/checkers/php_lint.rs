...
fn create_php_lint(toml: &toml::Value) -> anyhow::Result<Box<dyn crate::processors::Processor>> {
    crate::registries::deserialize_and_create(toml, |cfg| Box::new(SimpleChecker::new(cfg, SimpleCheckerParams { description: "Check PHP syntax using php -l", subcommand: Some("-l"), prepend_args: &[], extra_tools: &[] })))
}
