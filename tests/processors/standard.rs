...
#[test]
fn standard_valid_js() {
    if !tool_available("standard") {
        eprintln!("standard not found, skipping test");
        return;
    }
...
fn standard_incremental_skip() {
    ...
    assert!(
        stdout2.contains("[standard] Skipping (unchanged):"),
