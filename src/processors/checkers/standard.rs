...
fn create_standard(toml: &toml::Value) -> anyhow::Result<Box<dyn crate::processors::Processor>> {
    crate::registries::deserialize_and_create(toml, |cfg| Box::new(SimpleChecker::new(cfg, SimpleCheckerParams { description: "Check JavaScript style using standard", subcommand: None, prepend_args: &[], extra_tools: &["node"] })))
}
