...
#[test]
fn yq_valid_yaml() {
    if !tool_available("yq") {
        eprintln!("yq not found, skipping test");
        return;
    }
    ...
    fs::write(project_path.join("test.yaml"), "---\nname: test\nvalue: 42\n").unwrap();
...
fn yq_incremental_skip() {
    ...
    assert!(
        stdout2.contains("[yq] Skipping (unchanged):"),
