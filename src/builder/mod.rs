...
impl Builder {
    ...
    fn build_graph_with_processors_impl(&self, processors: &ProcessorMap, mode: GraphBuildMode, stop_after: BuildPhase, processor_filter: Option<&[String]>, verbose: bool) -> Result<(BuildGraph, PhaseTimings)> {
        ...
        let t = Instant::now();
        // Fixed-point discovery loop: after each pass, inject declared outputs
        // as virtual files so downstream processors can discover products for
        // files that don't exist on disk yet (generated by upstream processors).
        let max_passes = 10;
        ...
        for pass in 0..max_passes {
            ...
            for name in &active_processors {
                if mode == GraphBuildMode::ForClean {
                    processors[*name].discover_for_clean(&mut graph, &file_index)?;
                } else {
            ...
            if after == before {
                break; // No new products discovered
            }
            if pass + 1 < max_passes {
                // Collect all declared outputs and inject as virtual files
                ...
                if added == 0 {
                    break; // No new files to discover from
                }
                if phases_debug() {
    ...
    pub fn build_graph_filtered(
        ...
        for _pass in 0..10 {
            ...
            let outputs: Vec<PathBuf> = graph.products()[before..after]
                ...
                .flat_map(|p| p.outputs.iter().cloned())
                .collect();
            if file_index.add_virtual_files(&outputs) == 0 {
                break;
            }
        }
