...
#[test]
fn php_lint_valid_php() {
    if !tool_available("php") {
    ...
    fs::write(project_path.join("test.php"), "<?php\necho \"hello\";\n").unwrap();
...
fn php_lint_incremental_skip() {
    ...
    assert!(
        stdout2.contains("[php_lint] Skipping (unchanged):"),
