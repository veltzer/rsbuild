...
#[test]
fn perlcritic_valid_perl() {
    if !tool_available("perlcritic") {
        eprintln!("perlcritic not found, skipping test");
        return;
    }
...
#[test]
fn perlcritic_incremental_skip() {
    if !tool_available("perlcritic") {
        eprintln!("perlcritic not found, skipping test");
        return;
    }
    ...
    assert!(
        stdout2.contains("[perlcritic] Skipping (unchanged):"),
